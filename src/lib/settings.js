...
export const DEFAULT_SETTINGS = {
  ...
  // AI (ChatGPT 구독 계정. API 키 종량제를 쓰지 않는다)
  chatgpt: {
    // codex — codex CLI 를 부른다. `codex login` 으로 ChatGPT 계정에 로그인해 두면 된다. (권장)
    // web   — chatgpt.com 을 브라우저로 띄워 대화창에 물어본다. CLI 를 못 깔 때의 길.
    engine: 'codex',
    command: 'codex',
    model: '',                   // 비우면 CLI 기본 모델
    timeoutMs: 420000,           // 정보성 글은 길어서 넉넉히 잡는다.

    // 아래는 web 방식과 그림 생성에서 쓴다. (그림은 어느 방식이든 chatgpt.com 을 쓴다)
  ...
  post: {
    ...
    /*
     * 근거 자료를 못 구해도 **무조건 글을 쓴다.**
     *
     * 끄면 AI 가 "이 주제는 확인된 자료가 없어 쓸 수 없습니다" 라며 몇 문단짜리
     * 거절문을 보내고 그 주제는 건너뛴다. 실제로 그것 때문에 멀쩡한 주제가
     * 통째로 날아갔다. 켜 두면 과거 자료·널리 알려진 내용·합리적인 추정으로라도
     * 끝까지 쓰게 하고, 대신 **추정은 추정이라고 본문에 밝히게** 한다.
     *
     * 지어내면 안 되는 것은 그대로 막는다. 출처 URL 과 "2026년 취업률 87.3%"
     * 같은 조사 수치는 없으면 안 쓰는 것이 맞다. 그건 거절이 아니라 표현의 문제다.
     */
    neverRefuse: true,
  ...
  // 썸네일 이미지 생성 (ChatGPT 구독 계정으로 만든다. API 키가 필요 없다)
  image: {
    // 켜려면 ChatGPT 로그인만 되어 있으면 된다. 다만 대화창을 한 번 더 몰아야 해서
    // 글 한 편당 시간이 늘고 구독 등급의 그림 한도를 쓴다. 그래서 기본은 꺼짐.
    enabled: false,
    provider: 'chatgpt',         // ChatGPT 구독 계정 (chatgpt.com 대화창)

    // full    — 글자까지 포함한 완성 썸네일을 ChatGPT 가 통째로 그린다
    // overlay — 글자 없는 배경만 그리고 한글은 HTML 이 얹는다 (빠르고 안전)
    mode: 'full',

    // full 모드에서 글자가 깨졌는지 이미지를 다시 읽어 확인한다.
    // 깨졌으면 한 번 다시 그리고, 그래도 깨지면 HTML 썸네일로 물러선다.
    verifyText: true,
    retries: 1,                  // 글자가 깨졌을 때 다시 그려볼 횟수

    style: 'flat',               // flat | soft | photo | line (overlay 모드용)
    poster: 'bold',              // bold | clean | playful (full 모드용)
    timeoutMs: 300000,           // 대화창으로 그림을 받으려면 API 보다 오래 걸린다.
  },
 ...
 *
 * 지금은 비어 있다. 글도 그림도 **ChatGPT 구독 계정**으로 만들기 때문에
 * 설정 파일에 API 키를 담지 않는다. 로그인 정보는 설정이 아니라
 * 브라우저 프로필(data/chatgpt-profile)과 codex CLI 쪽에만 있다.
 * 나중에 키가 필요한 값이 생기면 여기에 경로를 한 줄 넣으면 된다.
 */
const SECRET_PATHS = [];
...
export function saveSettings(patch) {
  ...
  // 화면에서 되돌아온 마스킹 값(●●●●)으로 진짜 값을 덮어쓰지 않는다.
  ...
  // 블로그 아이디처럼 남이 볼 이유가 없는 값이 들어 있다.
  // 같은 컴퓨터의 다른 계정에서 못 읽게 한다.
  try {
...
/** 브라우저로 내려보낼 설정. 감출 값은 값을 빼고 "채워짐" 여부만 남긴다. */
export function publicSettings() {
  const stored = getSettings();
  const settings = structuredClone(stored);
  for (const [group, key] of SECRET_PATHS) {
    if (!settings[group]) continue;
    settings[group][`${key}Set`] = Boolean(stored[group]?.[key]);
    settings[group][key] = '';
  }
  return settings;
}
