...
import { checkChatGpt, runChatGpt } from './ai/chatgpt.js';
import {
  openChatGptLoginWindow, verifyChatGptSession, readChatGptSession, chatGptLogout,
  closeChatGptContext,
} from './ai/session.js';
import { MODELS, WEB_MODELS } from './ai/models.js';
...
import { generateBackground, pickAspectRatio, verifyKoreanText } from './content/imagegen.js';
...
app.get('/api/health', wrap(async (req, res) => {
  const chatgpt = await checkChatGpt();
...
/**
 * 글과 그림을 만드는 AI 는 ChatGPT 구독 계정을 쓴다.
 *
 * codex CLI 방식이면 터미널에서 `codex login` 을 한 번 하면 되고,
 * 그림 생성과 웹 방식은 chatgpt.com 세션이 필요하다. 네이버와 똑같이
 * 진짜 브라우저 창을 띄워 사용자가 직접 로그인하게 하고 세션만 넘겨받는다.
...
app.post('/api/chatgpt/login', wrap(async (req, res) => {
  ...
  openChatGptLoginWindow().catch((error) => logger.error(`ChatGPT 로그인 실패: ${error.message}`));
}));
...
app.post('/api/ai/test', wrap(async (req, res) => {
  const { engine, model, webModel } = getSettings().chatgpt;
  const wanted = engine === 'web' ? webModel : model;
  logger.step(
    `AI 연결 테스트 시작 (${engine === 'web' ? 'chatgpt.com 웹' : 'codex CLI'}`
    + `${wanted ? ` · ${wanted}` : ''})`,
  );
  try {
    const reply = await runChatGpt('"준비완료" 라고만 답하세요. 다른 말은 하지 마세요.', {
      systemPrompt: '당신은 짧게 답하는 도우미입니다.',
      timeoutMs: 120000,
    });
    logger.info(
      `AI 연결 테스트 성공 — ${reply.model || '기본 모델'}, `
      + `응답: ${reply.text.trim().slice(0, 40)}`,
    );
    res.json({
      ok: true,
      engine,
...
app.post('/api/image/test', wrap(async (req, res) => {
  ...
  if (!session.loggedIn) {
    res.json({
      ok: true,
      failed: true,
      message: 'ChatGPT 에 로그인되어 있지 않습니다. 1번 칸의 [ChatGPT 로그인 창 열기] 를 '
        + '눌러 구독 계정으로 로그인한 뒤 다시 시도해 주세요.',
      settings: publicSettings(),
    });
    return;
  }
  logger.step(`이미지 생성 테스트 시작 (ChatGPT 구독${session.plan ? ` · ${session.plan}` : ''})`);
...
function onReady(port) {
  ...
  // 그림 생성은 ChatGPT 구독 세션이 있어야 돈다. 글쓰기(codex CLI)는 이것과 별개다.
  const ai = readChatGptSession();
  if (ai.loggedIn) {
    logger.info(`저장된 ChatGPT 세션 있음${ai.plan ? ` · ${ai.plan}` : ''}`);
  } else if (getSettings().image.enabled || getSettings().chatgpt.engine === 'web') {
    logger.warn(
      'ChatGPT 로그인이 필요합니다. 대시보드 1번 칸에서 [ChatGPT 로그인 창 열기] 를 눌러주세요. '
      + '(로그인하지 않으면 썸네일은 HTML 로 만듭니다)',
    );
  }
}
