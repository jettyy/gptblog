...
import { AUTH_HINT } from '../ai/chatgpt.js';
...
async function processJob(job) {
  ...
  const post = await generatePost(job.topic, {
    ...
    onRoster: () => {
      updateJob(job.id, {
        status: STATUS.CENSUS,
        message: '순위를 매길 대상을 전수 조사하는 중...',
      });
    },
    onCompliance: () => {
  ...
  while (state.running) {
    ...
    if (step === 'discover') {
      ...
      try {
        ...
        if (error.authExpired) {
          state.paused = true;
          updateRequest(request.id, { message: 'ChatGPT 로그인이 풀려 대기 중입니다.' });
          logger.error(AUTH_HINT);
          broadcast();
          continue;
        }
    ...
    try {
      ...
      /*
       * ChatGPT 로그인이 풀렸다. 계속 돌리면 남은 주제가 **전부** 같은 이유로
      ...
      if (error.authExpired) {
        updateJob(job.id, {
          ...
          message: 'ChatGPT 로그인이 풀려 대기 중입니다.',
          detail: message,
