...
- 사실 여부가 걸린 소재인데 검색으로 근거를 전혀 못 찾은 것 (없는 제도나 일정 등)
  (다만 근거가 조금 부족하다고 후보에서 빼지는 마세요. 글은 알려진 내용과 추정으로도 씁니다)
...
export async function discoverTopics(bigTopic, { want, exclude = [], signal } = {}) {
   ...
   *
   * 예전에는 ChatGPT 호출이 한 번 어긋나면(시간 초과, JSON 이 아닌 응답, CLI 오류)
 ...
 *
 * "끊김 없이 영원히" 를 켜 뒀는데 ChatGPT 호출 한 번이 어긋났다고 멈추면
...
export async function discoverBigTopics({ want = 3, avoid = [], signal } = {}) {
  ...
  let reply = { data: null };
  try {
    reply = await runChatGptJson(buildBigTopicPrompt(seeds, Math.max(want + 2, 5)), {
