...
import {
  detectShape, normalizeRoster, decideRowCount, buildRosterPrompt, buildChunkPrompt, rosterKey,
} from '../src/content/ranking.js';
import {
...
import {
  looksAuthExpired, AUTH_HINT, runChatGpt, parseCodexOutput, extractJson,
} from '../src/ai/chatgpt.js';
...
test('전수 조사 응답에서 실제 이름만 골라낸다', () => {
  const roster = normalizeRoster({
    names: ['영진전문대학교', ' 영진전문대학교 ', '1. 대구보건대학교', '기타', '그 외', '', 'X'.repeat(100)],
    total: '130',
    totalConfident: true,
    exhausted: false,
    rankBasis: '재학생 규모와 취업률',
  });
  // 번호 붙은 항목은 번호를 떼고, 같은 이름은 한 번만, 묶음 표현은 버린다.
  assert.deepEqual(
    roster.names.slice(0, 2),
    ['영진전문대학교', '대구보건대학교'],
    '이름 정리가 안 됐습니다',
  );
  assert.ok(!roster.names.includes('기타'), '"기타" 가 한 줄을 차지했습니다');
  assert.ok(!roster.names.includes('그 외'), '"그 외" 가 한 줄을 차지했습니다');
  assert.equal(roster.names.length, 3, `중복이나 빈 값이 남았습니다: ${roster.names.join(', ')}`);
  assert.equal(roster.total, 130, '전체 개수를 숫자로 못 읽었습니다');
  assert.equal(roster.totalConfident, true);
  assert.equal(roster.rankBasis, '재학생 규모와 취업률');

  // 약칭과 정식 명칭이 띄어쓰기만 다르면 같은 대상으로 본다.
  assert.equal(rosterKey('대구 보건대학교'), rosterKey('대구보건대학교'));
});

test('있는 것보다 많이 요청하면 있는 것만큼만 줄을 세운다', () => {
  /*
   * 사용자가 바라는 동작이 이것이다.
   *   "2026 전문대 순위 TOP 50" → 50개보다 많으면 50까지
   *                             → 26개밖에 없으면 26개 전부로 표를 만든다
   * 없는 이름을 지어내 50줄을 채우면 글을 못 쓴다.
   */
  const asked = 50;

  // 26개뿐이고 다 훑었다 → 26개로 줄을 세운다.
  assert.equal(
    decideRowCount(asked, { names: new Array(26).fill('x').map((_, i) => `곳${i}`), exhausted: true }),
    26,
    '있는 것 전부로 줄을 세우지 않았습니다',
  );

  // 130개가 있다 → 요청한 50개까지만.
  assert.equal(
    decideRowCount(asked, { names: new Array(50).fill('x').map((_, i) => `곳${i}`), exhausted: false, total: 130 }),
    50,
    '요청한 개수를 넘겼거나 줄였습니다',
  );

  // 다 훑지는 못했지만 "이 범주는 원래 26개뿐" 이라고 확신하는 경우.
  assert.equal(
    decideRowCount(asked, { names: ['가', '나'], exhausted: false, total: 26, totalConfident: true }),
    26,
    '확신한 전체 개수를 안 따랐습니다',
  );

  // 확신이 없는 짐작값으로 줄이면 있는데도 안 쓰는 표가 된다. 그대로 50개로 간다.
  assert.equal(
    decideRowCount(asked, { names: ['가'], exhausted: false, total: 20, totalConfident: false }),
    50,
    '어설픈 짐작으로 표를 줄였습니다',
  );

  // 전수 조사가 아예 실패했으면(조사 결과 없음) 예전처럼 요청한 개수로 간다.
  assert.equal(decideRowCount(asked, null), 50);
  assert.equal(decideRowCount(asked, { names: [], exhausted: true }), 50, '빈손인데 표를 0행으로 만들었습니다');
});

test('전수 조사 프롬프트가 "지어내지 말라"고 못박는다', () => {
  const prompt = buildRosterPrompt('2026 전문대 순위 top50', 50, ['영진전문대학교']);
  assert.ok(prompt.includes('2026 전문대 순위 top50'), '주제가 안 들어갔습니다');
  assert.ok(prompt.includes('50개'), '목표 개수가 안 들어갔습니다');
  assert.ok(/지어내면 절대 안 됩니다/.test(prompt), '지어내기 금지 지시가 없습니다');
  assert.ok(prompt.includes('영진전문대학교'), '이미 받은 목록을 안 넘겼습니다');
  assert.ok(prompt.includes('rankBasis'), '순위 기준을 안 물어봤습니다');
  assert.ok(prompt.includes('exhausted'), '더 있는지를 안 물어봤습니다');
});

test('조사해둔 목록이 있으면 그 안에서만 표를 채우게 한다', () => {
  const headers = ['순위', '학교명', '강점 분야', '지역'];
  const roster = { names: ['영진전문대', '대구보건대', '연암공대'], exhausted: true };

  const prompt = buildChunkPrompt({
    topic: '2026 전문대 순위 top3',
    headers,
    start: 1,
    end: 3,
    existingNames: [],
  ...
  assert.ok(prompt.includes('영진전문대'), '조사해둔 목록이 안 들어갔습니다');
  assert.ok(/이 안에서만/.test(prompt), '목록 안에서 고르라는 지시가 없습니다');
  assert.ok(prompt.includes('취업률'), '순위 기준이 안 들어갔습니다');
  // 다 훑은 범주에는 "범위를 넓혀서 채우라" 고 하면 안 된다. 없는 이름을 지어내게 된다.
  assert.ok(!/범위를 넓혀/.test(prompt), '더 없는데 범위를 넓히라고 했습니다');
  assert.ok(/지어내면 안 됩니다/.test(prompt), '목록 밖 이름 금지 지시가 없습니다');

  // 이미 표에 들어간 이름은 후보에서 빠져야 한다. 안 빠지면 같은 곳이 두 번 들어간다.
  const second = buildChunkPrompt({
    topic: '2026 전문대 순위 top3',
    headers,
    start: 2,
    end: 3,
  ...
  // "고를 후보" 줄만 떼어 본다. 프롬프트 뒤쪽에는 "이미 나온 항목 (전부 제외)" 줄이
  // 따로 있어서, 프롬프트 전체를 훑으면 그 줄에 걸려 늘 실패한다.
  const pool = second.split('\n').find((line) => line.includes('이 안에서만')) || '';
  assert.ok(pool, '후보 목록 줄이 없습니다');
  assert.ok(!pool.includes('영진전문대'), '이미 쓴 이름이 후보에 남았습니다');
  assert.ok(pool.includes('대구보건대'), '남은 후보가 사라졌습니다');

  // 조사가 실패해 목록이 없으면 예전처럼 범위를 넓혀 채운다.
  const noRoster = buildChunkPrompt({
    topic: '전국 대학 순위',
    headers,
    start: 90,
    end: 100,
  ...
  assert.ok(/범위를 넓혀/.test(noRoster), '조사 결과가 없으면 범위를 넓혀야 합니다');
});
...
test('ChatGPT 로그인이 풀린 오류를 알아본다', () => {
  // codex CLI 가 실제로 내놓는 메시지. 이걸 못 알아보면 남은 주제가 전부 같은 이유로 실패한다.
  assert.ok(looksAuthExpired('Not logged in. Run `codex login` to authenticate.'));
  assert.ok(looksAuthExpired('ERROR: 401 Unauthorized'));
  assert.ok(looksAuthExpired('authentication_error'));
  assert.ok(looksAuthExpired('Your token has expired, please re-authenticate'));
  // 웹 방식에서 세션이 풀렸을 때 우리가 만드는 메시지도 알아봐야 한다.
  assert.ok(looksAuthExpired('ChatGPT 로그인이 필요합니다.'));
  // 다른 실패까지 로그인 문제로 보면 멀쩡한 대기열이 세워진다.
  assert.ok(!looksAuthExpired('codex CLI 를 찾을 수 없습니다'));
  assert.ok(!looksAuthExpired('Request timed out'));
  assert.ok(!looksAuthExpired('rate limit exceeded'));
  // 사람이 무엇을 해야 하는지가 안내에 들어 있어야 한다.
  assert.ok(AUTH_HINT.includes('codex login'), '무엇을 실행하라는 건지 안 적혀 있습니다');
  assert.ok(AUTH_HINT.includes('이어서 실행'), '그다음에 뭘 누르라는 건지 안 적혀 있습니다');
});

test('codex --json 출력에서 답과 검색 횟수를 뽑아낸다', () => {
  /*
   * codex CLI 는 판이 올라가면서 --json 이벤트 이름을 바꿨다.
   * 한 가지 모양만 읽으면 CLI 를 올린 순간 "응답이 비었습니다" 로 멈춘다.
   * 그래서 아는 모양을 전부 읽을 수 있어야 한다.
   */
  const legacy = parseCodexOutput([
    '{"msg":{"type":"task_started"}}',
    '{"msg":{"type":"agent_message","message":"{\\"ok\\":true}"}}',
  ].join('\n'));
  assert.equal(legacy.text, '{"ok":true}', '구판 모양을 못 읽었습니다');

  const modern = parseCodexOutput([
    '{"type":"item.completed","item":{"type":"web_search_call"}}',
    '{"type":"item.completed","item":{"type":"web_search_call"}}',
    '{"type":"item.completed","item":{"type":"agent_message","text":"답입니다"}}',
    '{"type":"turn.completed","usage":{"input_tokens":10,"output_tokens":5}}',
  ].join('\n'));
  assert.equal(modern.text, '답입니다', '신판 모양을 못 읽었습니다');
  assert.equal(modern.searches, 2, '웹 검색 횟수를 못 셌습니다');
  assert.equal(modern.tokens, 15);

  // 마지막 답을 써야 한다. 중간에 상황 설명을 흘리는 판이 있다.
  const twice = parseCodexOutput([
    '{"type":"agent_message","message":"잠시만요"}',
    '{"type":"agent_message","message":"최종 답"}',
  ].join('\n'));
  assert.equal(twice.text, '최종 답');

  // --json 이 막힌 판에서는 평문이 온다. codex 가 찍는 머리말은 걷어내야 한다.
  const plain = parseCodexOutput([
    '[2026-09-20T10:00:00] OpenAI Codex v0.9.0',
    '--------',
    'workdir: /home/user',
    'model: gpt-5.1-codex',
    '--------',
  ...
  assert.equal(plain.text, '{"title":"제목"}', '평문에서 머리말을 못 걷어냈습니다');
});

test('모델이 말을 덧붙여도 JSON 본체만 뽑아낸다', () => {
  // 대화창에서 읽어온 답에는 "복사" 같은 버튼 글자가 앞뒤로 붙는 일이 있다.
  assert.deepEqual(extractJson('```json\n{"a":1}\n```'), { a: 1 });
  assert.deepEqual(extractJson('알겠습니다.\n{"a":1}\n복사'), { a: 1 });
  assert.throws(() => extractJson('JSON 이 없습니다'));
});

await testAsync('codex 가 프롬프트를 읽기 전에 죽어도 프로그램이 안 죽는다', async () => {
  /*
   * 로그인이 풀린 codex 는 프롬프트를 다 읽기도 전에 끝나 버린다.
   * 그때 나는 write EPIPE 를 받아주지 않으면 그대로 uncaughtException 이 되어
   * **서버 전체가 죽는다.** 대시보드에는 "연결할 수 없음" 만 뜬다.
   */
  const win = process.platform === 'win32';
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'gptrush-check-'));
  const fake = path.join(dir, win ? 'fake.cmd' : 'fake.sh');
  fs.writeFileSync(fake, win
    ? '@echo off\r\necho Not logged in. Run `codex login` to authenticate. 1>&2\r\nexit /b 1\r\n'
    : '#!/bin/sh\necho "Not logged in. Run \\`codex login\\` to authenticate." >&2\nexit 1\n');
  if (!win) fs.chmodSync(fake, 0o755);

  const before = getSettings().chatgpt.command;
  saveSettings({ chatgpt: { engine: 'codex', command: fake, timeoutMs: 20000 } });

  let caught = null;
  try {
    // 프롬프트가 길어야 파이프에 한 번에 안 들어가서 EPIPE 가 제대로 재현된다.
    await runChatGpt('가'.repeat(200000), { timeoutMs: 20000 });
  } catch (error) {
    caught = error;
  } finally {
    saveSettings({
      chatgpt: {
        engine: DEFAULT_SETTINGS.chatgpt.engine,
        command: before,
        timeoutMs: DEFAULT_SETTINGS.chatgpt.timeoutMs,
      },
    });
